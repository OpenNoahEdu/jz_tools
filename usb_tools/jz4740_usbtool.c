 ...
 *                     \/            \/     \/    \/            \/
 * $Id: jz4740_usbtool.c 19406 2008-12-12 20:13:50Z gevaerts $
 *
...
int upload_app_noboot(usb_dev_handle* dh, int address, unsigned char* p, int len)
{
    int err;
    char buf[9];
    unsigned char* tmp_buf;
...
int send_rockbox(usb_dev_handle *dh)
{
    ...
    //_SET_ADDR(0x81024008);
    _SEND_FILE("onda.bin");
    _GET_CPU;
    _VERIFY_DATA("onda.bin", 0x080004000);
    //_VERIFY_DATA("onda.bin", 0x81024008);
    _GET_CPU;
    _FLUSH;
    _GET_CPU;
    _STAGE2(0x080004008);
...
int send_uboot(usb_dev_handle *dh)
{    
    int err, fsize;
    unsigned char *buffer, *buffer2;
    char cpu[8];
    ...
    _GET_CPU;
    _SET_ADDR(0x8000 << 16);
    _SEND_FILE("jz_xloader.bin");
    _GET_CPU;
    _VERIFY_DATA("jz_xloader.bin", 0x8000 << 16);
    _STAGE1(0x8000 << 16);
    _SLEEP(3);
    _GET_CPU;
    _SET_ADDR(0x80100000);
    _SEND_FILE("u-boot-boot-linux.bin");
    _GET_CPU;
    _VERIFY_DATA("u-boot-boot-linux.bin", 0x80100000);
    _GET_CPU;
    _FLUSH;
    _GET_CPU;
    _STAGE2(0x80100000);
...
int send_uboot_program_nand(usb_dev_handle *dh)
{    
    int err, fsize;
    unsigned char *buffer, *buffer2;
    char cpu[8];
    ...
    _GET_CPU;
    _SET_ADDR(0x80100000);
    _SEND_FILE("u-boot-program-nand.bin");
    _GET_CPU;
    _VERIFY_DATA("u-boot-program-nand.bin", 0x80100000);
    _GET_CPU;
    _FLUSH;
    _GET_CPU;
    _STAGE2(0x80100000);
...
int send_uboot_linux(usb_dev_handle *dh)
{
    int err, fsize;
    unsigned char *buffer, *buffer2;
    char cpu[8];
    ...
    _GET_CPU;
    _SET_ADDR(0x80100000);
    _SEND_FILE("u-boot.bin");
    _GET_CPU;
    _SET_ADDR(0x80600000);
    _SEND_FILE("uImage");
    _GET_CPU;
    _VERIFY_DATA("u-boot.bin", 0x80100000);
    _GET_CPU;
    _VERIFY_DATA("uImage", 0x80600000);
    _GET_CPU;
    _FLUSH;
    _GET_CPU;
    _STAGE2(0x80100000);
...
/* usb_set_configuration() calls are already done in Linux */
	...
	break;
        case 11:
            err = send_uboot_linux(dh);
        break;
	case 12:
	    err = send_uboot(dh);
        break;
	case 13:
	    err = send_uboot_program_nand(dh);
        break;
    }
...
void print_usage(void)
{
    ...
    fprintf(stderr, "\t\t11 -> send u-boot and linux kernel to SDRAM\n");
